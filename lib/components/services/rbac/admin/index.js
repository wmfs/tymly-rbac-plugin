...
class RbacAdmin {
  ...
  async grantPermission(roleId, resourceType, resourceName, action) {
    ...
    const grants = await this.grantsForOn(roleId, resourceName)
  ...
  async removePermission(roleId, resourceType, resourceName, action) {
    ...
    const revisedGrants = grants.filter(a => a !== action)

    await this.permissionModel.upsert({
      roleId: roleId,
      stateMachineName: resourceName,
      allows: revisedGrants
    }, { })
  } // removePermission

  async grantsForOn(roleId, stateMachineName) {
