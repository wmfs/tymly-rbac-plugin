...
class RbacService {
  async boot (options, callback) {
    try {
      ...
      caches.defaultIfNotInConfig('roleMemberships', 100)
      this.userMembershipsCache = caches.userMemberships
      this.roleMembershipsCache = caches.roleMemberships
  ...
  async listRoleUsers (roleId) {
    const cachedMembers = this.roleMembershipsCache.get(roleId)
    if (Array.isArray(cachedMembers)) return cachedMembers

    const members = await findRoleUsers(roleId, this.roleMembershipModel, this.rbac)
    this.roleMembershipsCache.set(roleId, members)
    return members
  } // listRoleMembers
  ...
  resetCache () {
    ...
    this.roleMembershipsCache.reset()
  }
  ...
  debug () {
    ...
    if (this.rbac.unknownRoles.length) {
      ...
      console.log('  ', this.rbac.unknownRoles.join(', '))
    }
  } // debug
} // RbacService
